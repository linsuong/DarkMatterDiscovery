...
As the constraints are applied, one can observe 2 regions in Fig. \ref{fig:l345_md1} where $\lambda_{345}$, and $m_{h_1}$ is present in. The first region is where $\lambda_{345} \approx 10^{-10} \text{ to } 10^{-3}$ where $m_{h_1} \approx 60 \gev$. This is because this region is the region where $\mhone = \frac{m_H}{2}$, $H$ being the Higgs boson - this causes resonant annihilation via the Higgs. Recall that $\lambda_{345}$ controls the coupling between $h_1$ and the Higgs, which governs the annihilation of $h_1 h_1 \rightarrow \text{SM}$ particles. The other region is from $\lambda_{345} \approx 10^{-10} \text{ to } 10^{-1}$,  with a bigger range of $m_{h_1} \approx 300 \useto 1000 \gev$. This region is 

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{big_plots_(low_dpi)/DM2_against_DMP_Omegah2.pdf}
    \caption{Caption}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{big_plots_(low_dpi)/DM3_against_MD2_Omegah2.pdf}
    \caption{Caption}
    \label{fig:enter-label}
\end{figure}

In Fig. \ref{fig:DM2_md1_main}, one can observe the effects of the constraints slowly shrinking the parameter space. \todo{discuss what CMB does to the data, the large gap in between, recombination, etc. should be able to find this written down somewhere}

Considering other plots, for example - $\mhp$ against $\mhone$; after applying all constraints, we end up with the following parameter space:

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{big_plots_(low_dpi)/MDP_against_MD1_Omegah2.pdf}
    \caption{Caption}
    \label{fig:enter-label}
\end{figure}

One might notice the relationship between $\mhp$ and $\mhone$, which is:
\begin{equation}
    \frac{\mhp}{\mhone} \approx 0.1
\end{equation}
...
\begin{figure}[H]
    ...
    \\
    \begin{subfigure}[b]{0.49\textwidth}
        \includegraphics[width=\textwidth]{branching_ratio_plots/hminus/Br__h_-__rightarrow_e_-__bar__nu__e_h_1____Delta_m____scale___Delta_m_3_.pdf}
        \caption{}
        \label{fig:Br(h-en),Dm3_scale}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.49\textwidth}
        \includegraphics[width=\textwidth]{branching_ratio_plots/hminus/__Delta_m_3___Delta_m____scale_Br__h_-__rightarrow_e_-__bar__nu__e_h_1__.pdf}
        \caption{}
        \label{fig:Br(h-en),Br_scale}
    \end{subfigure}
    \caption{Branching ratios of the process $h_2 \rightarrow h_1 e^+e$ for different mass differences $\Delta m_+$ and $\Delta m_3$, with $m_{h_1} =100$ GeV.}
