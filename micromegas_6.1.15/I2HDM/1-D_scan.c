...
// Define the range and step size for MD1
double MD1_min = 10, MD1_max = 1e3;  // Min and max values of MD1
double step_size = 1;  // Step size for the scan

for (double MD1 = MD1_min; MD1 <= MD1_max; MD1 += step_size) {
    // Update values based on the current MD1 value
    DMP =  1;  
    DM2 =  1;  
    ld345 = 0.001; 
    DM3 = DM2 - DMP;
    ...
    // Assign the values to micrOMEGAs model
    assignValW("MD1", MD1);
    assignValW("DMP", DMP);
    assignValW("DM3", DM3);
    ...
    // Find Higgs decay widths and branching ratios
    txtList HDecay;
