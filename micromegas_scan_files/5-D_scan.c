...
{  int err;
  ...
  //usleep(getpid() % 1000000); //delay to get unique seed
  //seed_rng_with_urandom();
  //srand(time(NULL) + getpid());

  /* ================the new piece of code starts here =================*/
  //run in parallel (code to add suffix to file)
   char filename[256];
    int counter = 0;
  
    // Start with "scan.dat" and add suffixes if needed
    snprintf(filename, sizeof(filename), "scan.dat");
    while (access(filename, F_OK) == 0) {
        counter++;
        snprintf(filename, sizeof(filename), "scan_%d.dat", counter);
    }

    FILE *fscan = fopen(filename, "w");
    if (!fscan) {
        perror("Error opening file");
        return EXIT_FAILURE;
    }

   int a=0,n_points=50000;
...
double MD1_min  =10,  MD1_max  =1e+4;
...
srand(time(NULL) + getpid()); // random seed for multiple instance running
       while(a<n_points){ /* setting parameter space - remember to close the loop in the end! */
 	        MD1 = rnd_slg(MD1_min,MD1_max); 
          DMP = rnd_slg(DMP_min,DMP_max);
	        DM2 = rnd_slg(DM2_min,DM2_max); /* DM2 is just a MD2-MD1 mass difference, which I convert to DM3 value below - also non negative which is nice*/
	...
	assignValW("ld",ld);
  	...
  	txtList HDecay;
	double HWidth=pWidth("H",&HDecay); //find pointer to pwidth
	double brH_DMDM = findBr(HDecay,"~h1,~h1"); //find branching decay - if Higgs decay to DM with large branching, it is excluded.
	
/* ========= the new piece of code end  here =============*/
...
//Omegah2 - important part
#ifdef OMEGA
{ int fast=1;
  double Beps=1.E-4, cut=0.01;
  double Omega;  //relic density 
  int i,err; 
  ...
  //if (Omega < 0.12024) {
    // Pass the relic density constraint
   //} else {
        // Exclude this point in the parameter space
        //continue; // Skip to the next iteration
    //}
   //if (isnan(Omega)) {
            // Skip the current iteration if the variable is NaN
            //printf("Skipping NaN for Omega \n"); //sometimes omega too big, lose prescision, give NaN
	    //continue;
        //}
...
{ 
  ...
  double FluxA[NZ],FluxE[NZ],FluxP[NZ]; //ignore - experiemtn dependant
...
printf("\n==== Indirect detection =======\n");  
  ...
  //flux of DM annihilation product if ratio bigger than 1, it is excluded.
...
{ char* expName; 
  ...
  PvalDD=pval;//my addition //if >0.1, excluded.
